...
fn handle_menu_id(app: &AppHandle, id: &str) {
    match id {
        ...
        "goCart" => {
            if let Some(win) = main_webview(app) {
                let _ = win.eval("window.location.assign('/cart')");
            }
        }
        "goCheckout" => {
            if let Some(win) = main_webview(app) {
                let _ = win.eval("window.location.assign('/checkout')");
            }
        }
        _ => {}
    }
...
pub fn build_menu(app: &AppHandle) -> tauri::Result<Menu<tauri::Wry>> {
    ...
    let catalog = MenuItemBuilder::with_id("goCatalog", "Catalog")
        .accelerator("CmdOrCtrl+1")
        .build(app)?;
    let cart = MenuItemBuilder::with_id("goCart", "Cart")
        .accelerator("CmdOrCtrl+2")
        .build(app)?;
    let checkout = MenuItemBuilder::with_id("goCheckout", "Checkout")
    ...
    let about = PredefinedMenuItem::about(app, Some("About rustashop"), None)?;
